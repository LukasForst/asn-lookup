...
// this is coming from https://gist.github.com/P-A-R-U-S/a090dd90c5104ce85a29c32669dac107
// and it is not optimized at all

func ipv4RangeToCidr(ipStart string, ipEnd string) (prefixes []*net.IPNet) {
	ranges, err := iPv4RangeToCIDRRange(ipStart, ipEnd)
	if err != nil {
		log.Fatal(err)
	}
...
func uInt32ToIPNet(iPuInt32 uint32) *net.IP {
	ip := net.IPv4(
		byte(iPuInt32>>24),
		byte((iPuInt32&0x00FFFFFF)>>16),
		byte((iPuInt32&0x0000FFFF)>>8),
		byte(iPuInt32&0x000000FF),
	)
	return &ip
}
